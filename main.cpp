...
int main(int argc, char **argv) {
  ...
  if (filename == "-l") filename = GetLastFilename();
  else SaveLastFile(filename);
  ...
  if (code.type == TypeOfSite::ARENA) {
    string response =
        SendCodeToArena(code.first.c_str(), code.second.c_str(), code.code, GetArenaAuth());
    string id = GetIDFromString(response);
    string url = URLFromID(id, ARENA_SUBMISSION_URL);
    OpenURLInBrowser(url);
  } else if (code.type == TypeOfSite::PESHO) {
    string response = SendCodeToPesho(code.first.c_str(), code.second.c_str(), code.code);
    string id = GetIDFromString2(response);
    char buffer[PESHO_SUBMISSION_URL.size() + 15];
    sprintf(buffer, PESHO_SUBMISSION_URL.c_str(), code.first.c_str(), id.c_str());
...
string SendCodeToArena(const char *competitionId, const char *canonicalCompetitionId, string code,
                       string bearerToken) {
  char url[ARENA_SUBMIT_URL.size() + 3];
  sprintf(url, ARENA_SUBMIT_URL.c_str(), competitionId, canonicalCompetitionId);
  printf("POST %s\n", url);
  cpr::Response r = cpr::Post(cpr::Url(url), cpr::Bearer(bearerToken), cpr::Body{code},
...
string SendCodeToPesho(const char *assigment, const char *task, string code) {
  char url[PESHO_SUBMIT_URL.size() + code.size() + 20];
  sprintf(url, PESHO_SUBMIT_URL.c_str(), assigment, task);
  printf("POST %s\n", url);
  auto auth = GetPeshoAuth();
  cpr::Response r =
      cpr::Post(cpr::Url(url), cpr::Authentication{auth.first, auth.second, cpr::AuthMode::BASIC},
...
Code GetCodeFromFile(const string filename) {
  ...
  if (firstLine.find(ARENA_ORIGIN_URL) != string::npos) {
